...
namespace SoilWrapper
{
    void SoilWrap_glTexImage2D(GLenum target, 
                                GLint level, 
                                ...
                                GLenum type, 
                                const GLvoid * pixels,
                                Soil_TextureHeader* header = NULL)
    {
        size_t rawSize = GetAllocSize( (void*)pixels );
        if( !header || rawSize == INVALID_SIZE )
        {
            return;
        }
        void* memory;

        header->m_isCompressed      = 0;
        header->m_target            = target;
        header->m_internalFormat    = internalformat;
        header->m_pixelType         = type;
        header->m_format            = format;
        ...
        header->m_mipSizes[level] = rawSize;

        if( level > header->m_levels )
        {
            header->m_levels = level;
        }

        // when building the images inside soil we actually store in offfsets the pointer to the memory;
        if( header->m_mipOffsets[level] != NULL )
        {
            void* memory =(void*) header->m_mipOffsets[level];
            Soil_Free( memory );
        }

        memory = Soil_Malloc( rawSize );
        memcpy( memory, pixels, rawSize );
        header->m_mipOffsets[level] = (char*) memory;
    
    }



 void   SoilWrap_glCompressedTexImage2D(GLenum target,
                                    GLint level,
    ...
    {
        size_t rawSize = GetAllocSize( (void*)data );
        if( !header || rawSize == INVALID_SIZE )
        {
            return;
        }
        ...
        header->m_mipSizes[level] = imageSize;
...
void SoilWrap_glTexImage2D( 
            ...
            Soil_TextureHeader* header = NULL)
{
    SoilWrapper::SoilWrap_glTexImage2D( target, 
                                        level, 
                                        internalformat,
                                        width, 
                                        height, 
                                    ...
                                    )
{
    ...
    SoilWrapper::SoilWrap_glCompressedTexImage2D(
                                                 target,
                                                 level,
                                                 internalformat,
                                                 width,
...
#undef glTexImage2D
#define glTexImage2D SoilWrap_glTexImage2D
#endif
...
// soil checks for some dxt so we need this
typedef void (APIENTRY * P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC) (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border, GLsizei imageSize, const GLvoid * data);
//P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC soilGlCompressedTexImage2D = NULL;

#define soilGlCompressedTexImage2D SoilWrap_glCompressedTexImage2D


//void soilGlCompressedTexImage2D(GLenum target,
//                                GLint level,
//                                GLenum internalformat,
//                                GLsizei width,
//                                GLsizei height,
...
enum 
{
     ...
     ,SOIL_CLAMP_TO_EDGE				= 0x812F
     ,SOIL_NORMAL_MAP					= 0x8511
     ,SOIL_REFLECTION_MAP				= 0x8512
     ,SOIL_TEXTURE_CUBE_MAP				= 0x8513
     ,SOIL_TEXTURE_BINDING_CUBE_MAP		= 0x8514
     ,SOIL_TEXTURE_CUBE_MAP_POSITIVE_X	= 0x8515
     ,SOIL_TEXTURE_CUBE_MAP_NEGATIVE_X	= 0x8516
     ,SOIL_TEXTURE_CUBE_MAP_POSITIVE_Y	= 0x8517
     ,SOIL_TEXTURE_CUBE_MAP_NEGATIVE_Y	= 0x8518
     ,SOIL_TEXTURE_CUBE_MAP_POSITIVE_Z	= 0x8519
     ,SOIL_TEXTURE_CUBE_MAP_NEGATIVE_Z	= 0x851A
     ,SOIL_PROXY_TEXTURE_CUBE_MAP		= 0x851B
...
enum
{
    SOIL_TEXTURE_RECTANGLE_ARB				= 0x84F5,
    SOIL_MAX_RECTANGLE_TEXTURE_SIZE_ARB		= 0x84F8
};
...
enum
{
     SOIL_RGB_S3TC_DXT1	    = 0x83F0
    ,SOIL_RGBA_S3TC_DXT1	= 0x83F1
    ,SOIL_RGBA_S3TC_DXT3	= 0x83F2
    ,SOIL_RGBA_S3TC_DXT5	= 0x83F3
};
...
unsigned int
	...
	(
        ...
        Soil_TextureHeader* nativeTextureHeader
	);
...
{
    memset(nativeTextureHeader, 0, sizeof(Soil_TextureHeader));
	/*	variables	*/
	unsigned char* img;
	...
	tex_id = SOIL_internal_create_OGL_texture(
			...
			GL_MAX_TEXTURE_SIZE, nativeTextureHeader );
	...
	)
{
	...
	tex_id = SOIL_create_OGL_single_cubemap(
			img, width, height, channels,
			face_order, reuse_texture_ID, flags, nativeTextureHeader
			);
	...
	)
{
	...
	sz = dw+dh;
	sub_img = (unsigned char *)Soil_Malloc( sz*sz*channels );
	...
	)
{
	...
	img = (unsigned char*)Soil_Malloc( width*height*channels );
		...
		(height > max_supported_size) )		/*	2^n for later down-sampling)	*/
	{
		...
		{
			/*	yep, resize	*/
			unsigned char *resampled = (unsigned char*)Soil_Malloc( channels*new_width*new_height );
			up_scale_image(
					img, width, height, channels,
	...
	if( (width > max_supported_size) || (height > max_supported_size) )
	{
		...
		resampled = (unsigned char*)Soil_Malloc( channels*new_width*new_height );
	...
	if( tex_id )
	{
		...
		if( DXT_mode == SOIL_CAPABILITY_PRESENT )
		{
			...
			if( DDS_data )
			{
				soilGlCompressedTexImage2D(
					...
					DDS_size, DDS_data, nativeTextureHeader );
		...
		} else
		{
			...
			glTexImage2D(
				opengl_texture_target, 0,
				internal_texture_format, width, height, 0,
				original_texture_format, GL_UNSIGNED_BYTE, img, nativeTextureHeader );
			check_for_GL_errors( "glTexImage2D" );
			/*printf( "OpenGL DXT compressor\n" );	*/
		}
		...
		if( flags & SOIL_FLAG_MIPMAPS )
		{
			...
			unsigned char *resampled = (unsigned char*)Soil_Malloc( channels*MIPwidth*MIPheight );
				...
				if( DXT_mode == SOIL_CAPABILITY_PRESENT )
				{
					...
					} else
					{
						...
						glTexImage2D(
							opengl_texture_target, MIPlevel,
							internal_texture_format, MIPwidth, MIPheight, 0,
							original_texture_format, GL_UNSIGNED_BYTE, resampled, nativeTextureHeader );
						check_for_GL_errors( "glTexImage2D" );
					}
				} else
				{
	...
	)
{
    ...
    pixel_data = (unsigned char*)Soil_Malloc( 3*width*height );
		...
		int loading_as_cubemap )
{
	...
	DDS_data = (unsigned char*)Soil_Malloc( DDS_full_size );
		...
		int loading_as_cubemap )
{
	...
	fseek( f, 0, SEEK_SET );
	buffer = (unsigned char *) Soil_Malloc( buffer_length );
	if( NULL == buffer )
	{
		result_string_pointer = "Soil_Malloc failed";
		fclose( f );
		return 0;
	}
