...
typedef __int32 soilU32;
typedef __int64 soilU64;

typedef struct 
{
static const soilU32 MAX_MIP_COUNT = 12;
    ...
    soilU32     m_isCompressed;

    char*       m_mipOffsets[MAX_MIP_COUNT];    //offsets to the mip data; miplevel "i" data is found between m_mipOffsets[i] and m_mipOffsets[i+1];
    soilU64     m_mipSizes[MAX_MIP_COUNT];      // size of each mip level in bytes
...
/**
	Frees the image data (note, this is just C's "Soil_Free()"...this function is
	present mostly so C++ programmers don't forget to use "Soil_Free()" and call
	"delete []" instead [8^)
**/
void
